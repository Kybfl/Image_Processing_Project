function sonuc = KCikarma(resim1, resim2)
    ...
    sonuc = zeros(size(resim1), 'like', resim1); % zeros(size(resim1)) girilmemesinin nedeni  
                                                    % bizim matrisimizin
                                                    % uint8 türünde olması
                                                    % gerektiği içindir.
                                                    % Tek parametreli
                                                    % olursa varsayılan
                                                    % double'dır.
    ...
    % Tüm piksellerde çıkarma - rgb resimler için tüm kanallara uygular
    for i = 1:size(resim1, 1)
        for j = 1:size(resim1, 2)
            for k = 1:size(resim1, 3)
                ...
                % Eğer resimler uint8 ise sınırla 
                if isa(resim1, 'uint8')
